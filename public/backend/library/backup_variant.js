(function ($) {
    ...
    HT.createVariant = () => {
        ...
        $('.variant-item').each(function () {
            ...
            // console.log('ID nhóm thuộc tính là: ', attributeCatalogueId)
            if (!attributeCatalogueId) {
                console.error('Không tìm thấy giá trị của attributeCatalogueId');
                return;  // Dừng tiếp tục nếu không có giá trị
            }
            let optionText = _this.find('.choose-attribute option:selected').text()
            let attribute = $('.variant-' + attributeCatalogueId).select2('data')
            if (attribute && attribute.length) {  // Kiểm tra nếu attribute đã được định nghĩa và có phần tử
    ...
    //             { name: 'variant[file_name][]', class: 'variant_filename', value: variant.barcode[index] },
