(function ($) {
    ...
    HT.createVariant = () => {
        ...
        //CẤU HÌNH LẠI MẢNG BẰNG TÍCH ĐỀ CÁC
    ...
    HT.renderTableHtml = (attributes, attributeTitle, variants) => {
        ...
        for (let j = 0; j < attributes.length; j++) {
            ...
            html = html + '<td>'
            html = html + '<span class="image img-cover"><img src="https://cdn.vuahanghieu.com/unsafe/0x900/left/top/smart/filters:quality(90)/https://admin.vuahanghieu.com/upload/product/2019/05/nuoc-hoa-armaf-club-de-nuit-intense-for-man-105ml-5cecf9ac49316-28052019160444.jpg" alt=""></span>'
            html = html + '</td>'
            let attributeArray = []
    ...
    HT.updateVariantHtml = () => {
        ...
        html = html + ' <input type="text" disabled name="variant_quantity" value="0" class="form-control disabled int" > '
        html = html + ' </div>'
        html = html + ' <div class="col-lg-3">'
        html = html + ' <label for="" class="control-label">DTC</label>'
        html = html + ' <input type="text" id="variantQuantity" name="variant_sku" value="" class="form-control text-right" > '
        html = html + ' </div>'
        html = html + ' <div class="col-lg-3">'
        html = html + ' <label for="" class="control-label">Giá</label>'
        html = html + ' <input type="text" id="variantQuantity" name="variant_price" value="0" class="form-control int" > '
        html = html + ' </div>'
        html = html + ' <div class="col-lg-3">'
        html = html + ' <label for="" class="control-label">Barcode</label>'
        html = html + ' <input type="text" id="variantQuantity" name="variant_barcode" value="" class="form-control text-right" > '
        html = html + ' </div>'
        html = html + ' </div>'
        html = html + ' </div>'
        html = html + ' </div>'
